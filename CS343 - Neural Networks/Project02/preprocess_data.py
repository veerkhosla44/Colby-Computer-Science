...
def preprocess_stl(imgs, labels):
    ...
    standardized_imgs = (flat_imgs - np.mean(flat_imgs, axis=0)) / np.std(flat_imgs, axis=0)
    
    return standardized_imgs, labels.astype(int)
...
def load_stl10(n_train_samps=3500, n_test_samps=500, n_valid_samps=500, n_dev_samps=500):
    ...
    processed_imgs, processed_labels = preprocess_stl(stl_imgs, stl_labels)
    return create_splits(processed_imgs, processed_labels, n_train_samps, n_test_samps, n_valid_samps, n_dev_samps)
